function LambdaSearchBMA(NUM_RUNS, FOLD, GCMDataFile, TargetDataFile, PartitionsDataFile,outputFileName)
    ...
    for run = 1:NUM_RUNS
        ...
        for lambda = lambda_values;
            ...
            for n = 1:FOLD
                ...
                % Load PEB results
                load(sprintf('PEBs/BMA_search_PEB_run%d_fold%d.mat', run, n), 'BMA')

                % Extract relevant BMA information
                TrainEP = abs(full(BMA.Ep));
                TrainEP = reshape(TrainEP, 9, 9, 2);
                TrainEP = TrainEP(:, :, 2);
                TrainEP = TrainEP - diag(diag(TrainEP));

                TrainCP = sqrt(diag(full(BMA.Cp)));
                ...
                for j = 1:length(DcmTrain)
                    TmpEP = reshape(DcmTrain{j}.Ep.B(:,:,2) .* Inx, 1, []);
                    TmpEP(TmpEP == 0) = [];
                ...
                for k = 1:length(DcmTest);
                    TestTmpEP = reshape(DcmTest{k}.Ep.B(:,:,2) .* Inx, 1, []);
